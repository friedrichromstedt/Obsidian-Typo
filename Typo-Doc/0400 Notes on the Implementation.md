...
Using blank lines between code blocks and other continuous text is generally encouraged: The *margins* around code blocks in *reading mode* are styled in such a way, that they correspond to single blank lines above and below of such a code block in *editing mode*. The blank lines of markdown are not required, but placing them in the markdown code in this way improves readability and brings the live mode closer to the preview mode.
